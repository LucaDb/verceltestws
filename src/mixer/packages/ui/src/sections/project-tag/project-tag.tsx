...
export type IProjectTag = IComponent & {
};
...
export const ProjectTag = ({ item, index = 0 }: ILazyableProps<IProjectTag>) => {
  ...
  const groups = useMemo(() => {
    ...
    projects?.forEach(item => {
      item.tag.forEach(tag => {
        let group = groups.find(x => x.id === tag.id);
        if (!group) {
          group = Object.assign({ items: [] }, tag);
          groups.push(group);
        }
  ...
  const classNames = getClassNames('project-tag');
