...
export type ICrudMethod = 'get' | 'post' | 'put' | 'patch' | 'delete' | string;

export type IHandler = (request: NextApiRequest, response: NextApiResponse) => Promise<any>;

export type IApiHandler = Record<ICrudMethod, IHandler>;
...
export function apiHandler(handler: IApiHandler): any {
  return async (request: NextApiRequest, response: NextApiResponse): Promise<any> => {
    const method: ICrudMethod = (request.method || 'GET').toLowerCase() as ICrudMethod;
