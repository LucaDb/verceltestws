import { IContact, IContactFormSubmit } from '@websolute/models';
import { ContactForm, useSummary } from '@websolute/ui';
...
export default function Contact({ layout, page, params }: PageProps<IContact>) {
  useSummary(page);
  const [submitted, setSubmitted] = useState(false);
  const onSubmit = (value: IContactFormSubmit) => {
    setSubmitted(true);
    window.scrollTo(0, 0);
  };
  return (
            ...
            <Grid.Row>
              <Grid gridColumnStartSm="3" gridColumnEndSm="11" paddingSm="0 var(--spacer-sm)" padding="0" alignItems="center" textAlign="center">
                ...
                <Text variant="heading30" marginBottom="90px" maxWidth="50ch">
                  Abbiamo ricevuto la tua richiesta di progetto! Ci metteremo in contatto il prima possibile. Buona giornata!
                </Text>
      ...
      {!submitted && (
        <Section padding="180px 0 90px 0" paddingMd="360px 0 180px 0">
          <Container>
            ...
            <ContactForm options={page.options} onSubmit={onSubmit}></ContactForm>
          </Container>
        </Section>
      )}
    </Main>
...
export async function getStaticProps(context: IStaticContext) {
  ...
  const page = await getPage<IContact>('contact', id, market, locale);
  if (!page) {
    return {
      notFound: true,
      revalidate: true,
