import { IIndustryDetail } from '@websolute/models';
...
export default function IndustryDetail({ layout, page, params }: PageProps<IIndustryDetail>) {
  useSummary(page);
...
export async function getStaticProps(context: IStaticContext) {
  ...
  const page = await getPage<IIndustryDetail>('industry_detail', id, market, locale);
  if (!page) {
    return {
      notFound: true,
      revalidate: true,
...
export async function getStaticPaths() {
  const paths = await getStaticPathsForSchema('industry_detail');
  return {
    paths,
